...
export async function main(ns: DeepReadonly<NS>): Promise<void> {
    ...
    if (corp.funds >= SEED_MONEY) {
        if (!buyUpgrade("Warehouse API")) return;
        if (!buyUpgrade("Office API")) return;
    }

    if (!buyUpgrade("Smart Supply")) return;
    ...
    async function assignOrHire(div: string, city: CityName, pos: string, num: number) {
        ...
        if (!ns.corporation.setAutoJobAssignment(div, city, pos, num)) {
            ...
            ns.print(`assignOrHire(${div}, ${city}, ${pos}, ${num}) failed with: toHire = ${toHire}, employeeJobs = `
                + `${office.employeeJobs[pos as keyof typeof office.employeeJobs]} / ${office.employeeJobs.Unassigned}!`);
        }
    }
    ...
    async function considerInvest(maxInvestNo: number) {
    ...
    async function setupDiv1() {
        ...
        /** @type {[string, number][]} */
        const INIT_MATERIALS: [string, number][] = [
            ["Hardware", 125],
            ["AI Cores", 75],
            ["Real Estate", 27e3],
        ];
        ...
        if (ns.corporation.hasUnlock("Office API")) {
            for (const city of Object.values(ns.enums.CityName)) {
                ...
                for (const [pos, targ] of NINE_EMPLOYEE_DIV) {
                    if (office.employeeJobs[pos as keyof typeof office.employeeJobs] < targ)
        ...
        const MATERIALS2: [string, number, number][] = [
            ["Hardware", 2800, 2675],
            ["Robots", 96, 96],
            ["AI Cores", 2520, 2445],
