...
/** @param {NS} ns */
export function parseConfig<K extends string, P, T = { [N in K]: P }>(ns: DeepReadonly<NS>, filename: string, struct: T): T {
    let json = struct;
    if (ns.fileExists(filename)) {
		json = JSON.parse(ns.read(filename), (k, value: unknown) => {
            if (k in struct) {
