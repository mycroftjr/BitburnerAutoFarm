...
export async function main(ns: NS): Promise<void> {
	...
	let netManager = false; let serverManager = false;
	/** @type {[number, ...string[]]} */
			...
			if ((ns.getPurchasedServers().includes(server) || info('NPR', server) <= exes.length) && prev != server) {
				if (!ns.getPurchasedServers().includes(server)) {
					for (const hack of exes) {
						(ns[hack as keyof NS] as (server: string) => void)(server)
					}
					ns.nuke(server)
				}
		...
		for (const host of hosts) {
			...
			if (hType !== HType.Weaken && fRam() >= 1.75) {
				...
				if (growThreads >= remainingThreads()) {
					...
					const finalGrowThreads = Math.min(growThreads, remainingThreads(),
					...
					if (finalGrowThreads > 0) {
						...
						const finalWeakenThreads = Math.min(weakenThreads, remainingThreads(),
	...
	async function hnManager() {
		...
		for (let i = 0; i < ns.hacknet.numNodes(); i++) {
			for (const part of ['Level', 'Ram', 'Core']) {
				if (checkM((ns.hacknet['get' + part + 'UpgradeCost' as keyof typeof ns.hacknet] as (index: number, n: number) => number)(i, 1), 20)) {
	...
	while (true) { // Keeps everything running once per second
		...
		if (i++ >= 60) {
			try { ns.run('autoSolver.js') } catch { }
			i = 0;
		}
		log()
